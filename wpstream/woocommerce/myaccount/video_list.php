<!--
	My Account > Video list (WpStream template override).
	Lists the current customer's purchased video-on-demand items (and VOD subscriptions).
...
<?php
 ...
 // Prefer the theme's own purchased-video renderer when it is available.
 if (function_exists('wpstream_theme_purchased_video_list')) {
        ...
        foreach( $customer_orders as $order_data){
            ...
            foreach ( $items as $item ) {
                ...
                // Render only video-on-demand products (or subscriptions flagged as non-live).
                if( $term_list[0]->name=='video_on_demand' ||   ( $term_list[0]->name=='subscription' && $is_subscription_live_event=='no') ){
                    ...
                    // Output the product card: thumbnail, title and a "watch the video" link.
