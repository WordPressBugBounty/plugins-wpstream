<?php
/**
 * Category unit template (type 1): a card showing a category's name, tagline,
 * item count and featured image. Expects $term_id (and optionally $item_height,
...
<!-- Category card wrapper (type 1): whole card is clickable via data-link to the term archive. -->
<div class="wpstream_category_unit_wrapper_type1  wpstream_category_unit_link <?php echo esc_attr($wrapper_class);?> "  data-link="<?php echo esc_url($term_link); ?>" <?php echo esc_attr($item_height_style);?> >
    

     <!-- Text details column: title, optional tagline and item count. -->
    ...
    <!-- Image tile: featured-image background with a clickable cover overlay. -->
    <div class="wpstream_category_unit_item col" data-link="<?php echo esc_attr($term_link);?>"    style="<?php echo trim($inline_style);?>" >
        <div class="wpstream_category_unit_item_cover" data-link="<?php echo esc_attr($term_link);?>">
        </div>
    </div> 
