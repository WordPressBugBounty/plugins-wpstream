 ...
 *
 * The dashboard "Start Streaming" screen: channel creation CTA, a channel
 * selector, the live-streaming controls, and the details panel for the selected
 * channel (title, description, price, taxonomies, trailer/preview videos and the
 * image gallery). Also loads the edit-channel modal and localizes the uploaders.
 *
 * @package wpstream-theme
 */

// Only users allowed to stream may see this screen; bail otherwise.
if ( ! wpstream_check_if_user_can_stream()) {
	return;
}
?>
    <!-- Start Streaming screen wrapper. -->
    <div class="wpstream-dashboard-start-streaming">
        <!-- Header: page title and create-channel CTA. -->
        ...
        <div class="wpstream-theme-dashboard-select-channel">
            ...
            <!-- Inline area for channel-switch AJAX notifications. -->
			...
			<?php
			// Current user and their saved streaming-channel selection.
            ...
            <div class="wpstream_theme_dashboard_channel_selector_wrapper">
				<?php
				// Output the dropdown list of the user's channels.
			...
			<?php
			// Output the start/stop live-streaming controls.
			...
			// Gather the selected channel's title, post, description, thumbnail and price.
			...
			<!-- Load the edit-channel modal markup. -->
        ...
        <div class="wpstream-theme-dashboard-chanel-details">
            ...
            <div class="wpstream-theme-dashboard-chanel-details__description wpstream-dashboard-details">
                ...
                <!-- Channel description text container. -->
			...
			<?php
			// Build the price block only when WooCommerce is available.
			if (  function_exists( 'get_woocommerce_currency_symbol' ) ) {
				// Hide the price row unless the channel is a paid product.
				$extra_style=" display:none ";
				if( 'product' === get_post_type( $current_selection ) ){
					$extra_style ='';
				}
				?>
                <!-- Price row (hidden unless the channel is paid). -->
                <div class="wpstream-dashboard-details wpstream-dashboard-details_price" style="<?php echo esc_attr($extra_style);?>" >
                    ...
                    <div class="wpstream_account_details_value">
                        ...
                        <div class="wpstream_account_details_value">
							<!-- Formatted price, or a 'not set' placeholder. -->
							<span id="wpstream_channel_price">
								<?php
								// No price shows a placeholder; otherwise the currency-formatted price.
								if ( '' === $price ) {
			...
			<?php
			// Load the trailer video, then render it or a 'no trailer' notice.
			...
			<?php
			// Load the preview video, then render it or a 'no preview' notice.
            ...
            <!-- Channel image gallery block. -->
            <div class="wpstream-theme-dashboard-chanel-gallery">
				<?php
				// Load the channel's gallery images.
				$gallery_images = wpstream_theme_return_image_gallery( $current_selection );

				?>

				<!-- Empty gallery shows a placeholder title; otherwise the gallery title. -->
                ...
                <!-- Rendered gallery image list. -->
                <div class="wpstream-theme-dashboard-chanel-gallery__list">
					<!-- Output the gallery image markup. -->
